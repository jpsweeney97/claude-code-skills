...
| `apply-destructive` | Execute approved destructive actions. | file deletion via reversible mechanism, local branch deletion |
...
- Follow repo-specific branch conventions when they are explicit. Otherwise use `cleanup/YYYY-MM-DD-HHMMSS`.
...
3. Delete approved files using a reversible mechanism (e.g., `trash` on macOS, `trash-put` on Linux, `gio trash` on GNOME).
...
- Never use `rm` or `git clean -fd`; use a reversible deletion mechanism (e.g., `trash`, `trash-put`, `gio trash`) for filesystem deletion.
...
Cleanup branch: cleanup/2026-03-20-143052

Lane results:
  untracked-and-ignore:
    .gitignore patterns added: 3
    files deleted (reversible): 0
  commit-shaping:
    commits created: 4
...
| Running `git clean -fd` because the repo looks messy | It turns cleanup into irreversible deletion. | Preview first and use a reversible deletion mechanism only for explicitly approved files. |
